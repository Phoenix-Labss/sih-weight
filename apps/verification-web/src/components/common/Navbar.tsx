...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, allowedTabs, onLogout }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 select-none shadow-card">
        ...
        <div className="flex items-center gap-3 text-slate-500 shrink-0">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="rounded border border-slate-300 bg-white px-1.5 py-0.5 text-xs font-semibold text-gov-navy transition-colors hover:bg-slate-50"
        ...
        <div className="flex items-center justify-between gap-3 lg:gap-6">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-left">
              ...
              <div className="leading-tight min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-amber-900 bg-amber-100 border border-amber-300 px-1.5 py-0.2 rounded uppercase shrink-0">
                    {user?.actorRole}
                  </span>
                </div>
            ...
            <button
              type="button"
              onClick={onLogout}
              className="px-3.5 py-1.5 sm:py-2 rounded bg-red-700 hover:bg-red-800 active:bg-red-900 text-white font-bold text-xs sm:text-sm shadow-card transition-colors flex items-center gap-1.5 cursor-pointer"
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <nav className="flex items-center flex-wrap lg:flex-nowrap">
            {primaryTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-5 sm:px-6 py-2.5 sm:py-3 font-semibold flex items-center gap-2 transition-colors cursor-pointer shrink-0 ${
                    isActive
            ...
            <button
              ...
              className={`px-4 sm:px-5 py-2.5 sm:py-3 font-semibold flex items-center gap-1.5 cursor-pointer ml-auto transition-colors ${
                activeTab === 'public'
                  ? 'bg-amber-400 text-slate-950'
      ...
      {mobileMenuOpen && (
        <div className="xl:hidden bg-gov-navy border-t border-slate-800 px-4 py-2 space-y-1 animate-fade-in">
          {allowedTabs.map((tab) => (
            <button
              key={tab}
              type="button"
            ...
            >
              ...
              {activeTab === tab && <span className="text-xs uppercase font-bold">Active</span>}
            </button>
          ))}
