...
import { Lock, ShieldCheck, CheckCircle2, Key, Stamp, AlertTriangle, QrCode } from 'lucide-react';
...
export const CertificateSignModal: React.FC<CertificateSignModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && session) {
      ...
      api.stamps
        .listSessionStamps(user.tenantId, session.session_id)
        .then((stamps) => {
    ...
    >
      {!issuedCert ? (
        ...
        <div className="space-y-4 text-center py-2">
          ...
          <div>
            <h4 className="text-lg font-bold text-gov-navy">Certificate Minted &amp; Cryptographically Signed</h4>
            <p className="text-xs text-slate-500 mt-1">
              The digital certificate is published and accessible for public QR verification.
            </p>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs max-w-md mx-auto text-left space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono font-bold text-emerald-800">
                {existingStamps.length > 0 ? `#${existingStamps[0].seal_identification_number}` : 'Affixed on Calibration Port'}
              </span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="pt-2 flex justify-center gap-3">
            <button
              onClick={handleDone}
              className="px-8 py-2.5 rounded-lg bg-gov-navy text-xs font-bold text-white hover:bg-slate-800 transition-colors shadow-2xs cursor-pointer"
            >
